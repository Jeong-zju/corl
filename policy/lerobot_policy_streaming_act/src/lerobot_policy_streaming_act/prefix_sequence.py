...
def select_full_prefix_positions(
    *,
    sequence_length: int,
    target_length: int,
) -> list[int]:
    return select_prefix_positions(
        sequence_length=sequence_length,
        prefix_train_max_steps=target_length,
...
def build_prefix_sequence_input_features(
    *,
    ...
    use_path_signature: bool,
    use_delta_signature: bool,
    include_prefix_images: bool = True,
...
class PrefixSequenceDataset(torch.utils.data.Dataset):
    ...
    def _load_current_item_without_videos(self, idx: int) -> dict[str, Tensor | Any]:
        ...
        self._ensure_current_signature_features(item)
        return item

    def _ensure_current_signature_features(
        self,
        item: dict[str, Tensor | Any],
    ) -> None:
        required_keys = []
        if self.use_path_signature:
            required_keys.append(PATH_SIGNATURE_KEY)
        if self.use_delta_signature:
            required_keys.append(DELTA_SIGNATURE_KEY)
        ...
        if "index" not in item:
            raise KeyError(
                "Current item must include `index` before fetching missing signature features."
            )

        absolute_index = int(
            item["index"].item() if isinstance(item["index"], torch.Tensor) else item["index"]
        )
        signature_cache_reader = getattr(self.base_dataset, "_signature_cache_reader", None)
        ...
        query_result = self.base_dataset._query_hf_dataset(
            {key: [absolute_index] for key in remaining_keys}
        )
        for key in remaining_keys:
            value = query_result.get(key)
            if value is None:
                raise KeyError(
                    f"Could not fetch required current-step signature feature `{key}`."
                )
            tensor = torch.as_tensor(value)
            item[key] = tensor[0] if tensor.ndim >= 2 and tensor.shape[0] == 1 else tensor

    def _augment_current_item_with_delta_queries(
        self,
        item: dict[str, Tensor | Any],
        *,
        abs_idx: int,
        ...
        delta_indices = getattr(self.base_dataset, "delta_indices", None)
        if delta_indices is None:
            return

        query_indices, padding = self.base_dataset._get_query_indices(abs_idx, ep_idx)
        query_result = self.base_dataset._query_hf_dataset(query_indices)
        item.update(padding)
    ...
    def _query_prefix_tensors(
        ...
        *,
        ...
        if self.use_path_signature and history_abs_indices:
            query_indices[PATH_SIGNATURE_KEY] = history_abs_indices
        if self.use_delta_signature and history_abs_indices:
    ...
    def __getitem__(self, idx: int) -> dict[str, Tensor | Any]:
        cache_covers_video_cameras = all(
            self._camera_uses_prefix_image_cache(camera_key)
            for camera_key in self.camera_keys
            if camera_key in self.base_dataset.meta.video_keys
        )
        can_prefetch_video_once = (
            len(self.base_dataset.meta.video_keys) > 0
            and not cache_covers_video_cameras
            and getattr(self.base_dataset, "delta_indices", None) is None
        )
        can_load_current_item_without_videos = bool(
            can_prefetch_video_once or cache_covers_video_cameras
        )
        item = (
            self._load_current_item_without_videos(idx)
            if can_load_current_item_without_videos
        ...
        if can_load_current_item_without_videos:
            self._augment_current_item_with_delta_queries(
                item,
                abs_idx=abs_idx,
                ep_idx=ep_idx,
            )
        ...
        elif cache_covers_video_cameras:
            for camera_key in self.camera_keys:
                if camera_key not in self.base_dataset.meta.video_keys:
                    continue
                current_frame = self.prefix_image_cache_reader.get(camera_key, abs_idx)
                if self.base_dataset.image_transforms is not None:
                    current_frame = self.base_dataset.image_transforms(current_frame)
                item[camera_key] = current_frame
        item.update(
