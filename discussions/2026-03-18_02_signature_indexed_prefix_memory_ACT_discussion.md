...
- [x] Prompt 2：补齐 prefix-sequence 训练基础设施
...
- [x] Prompt 3：实现 Visual Prefix Memory Only 基线
...
10. 优先先支持 `panda_route` 和 `braidedhub`，其它环境至少不要被破坏。
...
- [x] Prompt 4：实现完整 Signature-Indexed Prefix Memory ACT
...
9. 优先先让 `braidedhub` 跑通；如果 `h_shape` / `panda_route` 也能兼容更好。
