...
TRAIN_OUTPUT_ROOT="${TRAIN_OUTPUT_ROOT:-outputs/train/panda_route_streaming_act}"
RUN_TAG="${RUN_TAG:-$(date +%Y%m%d_%H%M%S)}"
OUTPUT_DIR="${OUTPUT_DIR:-outputs/eval/panda_route_streaming_act/${RUN_TAG}}"
...
if [[ $# -gt 0 && ( "${1}" == "-h" || "${1}" == "--help" ) ]]; then
  "${PYTHON_BIN}" scripts/eval_policy.py --env panda_route --policy streaming_act "$@"
  exit 0
fi
