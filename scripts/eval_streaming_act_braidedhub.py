...
DEFAULT_OUTPUT_DIR = Path("outputs/eval/braidedhub_fourstart_streaming_act")
...
def compute_logsignature_np(window: np.ndarray, sig_depth: int) -> np.ndarray:
    ...
    path = torch.from_numpy(window).unsqueeze(0)
    with torch.no_grad():
...
def detect_branch_mismatch(
    ...
    """Return mismatch details if the rollout enters a task-incompatible branch."""
...
def build_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate a Streaming ACT policy in the braidedhub implicit-cue map "
    ...
    parser.add_argument(
        ...
        help="Backend for online path-signature computation during eval.",
    )
...
def main() -> None:
    ...
    if cfg.use_path_signature:
        ...
        if cfg.signature_depth <= 0:
            raise ValueError(
                f"Invalid cfg.signature_depth={cfg.signature_depth}. Must be > 0."
            )
    ...
    for ep_idx, task_id in enumerate(task_schedule):
        ...
        trajectory = [state_xy]
        state_history = deque() if cfg.use_path_signature else None
        last_info = {
            **env.last_info,
